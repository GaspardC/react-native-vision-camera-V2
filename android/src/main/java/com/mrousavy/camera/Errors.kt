...
class ParallelVideoProcessingNotSupportedError(cause: Throwable) : CameraError(
  "device",
  "parallel-video-processing-not-supported",
  "The given LEGACY Camera device does not support parallel " +
    ...
    "See https://react-native-vision-camera.com/docs/guides/devices#the-supportsparallelvideoprocessing-prop for more information.",
  cause,
)
...
class VideoEncoderError(cause: Throwable?) : CameraError(
  "capture",
  "encoder-error",
...
class InvalidVideoOutputOptionsError(cause: Throwable?) : CameraError(
  "capture",
  "invalid-video-options",
...
class RecorderError(cause: Throwable?) : CameraError(
  "capture",
  "recorder-error",
...
class NoValidDataError(cause: Throwable?) : CameraError(
  "capture",
  "no-valid-data",
...
class InactiveSourceError(cause: Throwable?) : CameraError(
  "capture",
  "inactive-source",
...
class InsufficientStorageError(cause: Throwable?) : CameraError(
  "capture",
...
class FileSizeLimitReachedError(cause: Throwable?) : CameraError(
  "capture",
